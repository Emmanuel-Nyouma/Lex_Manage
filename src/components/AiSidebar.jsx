...
const AiSidebar = ({ 
  ...
  const getGreeting = (view) => {
    if (view && view.includes('cases')) {
      return "I'm viewing your cases. Ask me to summarize a case, draft documents, or analyze evidence.";
    } else if (view && view.includes('documents')) {
      return "I can help you search, summarize, or analyze your documents.";
    }
    return "Hello, I am LexAssist. How can I help you today?";
  };

  const [chatHistory, setChatHistory] = useState([
    { id: 1, sender: 'ai', text: getGreeting(currentView), isRich: false }
  ]);

  // Update greeting when view changes if chat has no history
  useEffect(() => {
    if (chatHistory.length === 1 && chatHistory[0].sender === 'ai') {
      setChatHistory([{ id: 1, sender: 'ai', text: getGreeting(currentView), isRich: false }]);
    }
  }, [currentView]);
      ...
      <>
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 id={dialogId} className="font-bold text-slate-900 dark:text-white text-sm">LexAssist AI</h3>
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase">Active</span>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="p-2 text-slate-500 dark:text-slate-300 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all"
          ...
          {chatHistory.length <= 1 && (
            <div className="py-8 px-4 text-center animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="w-16 h-16 bg-amber-50 dark:bg-amber-900/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-amber-100 dark:border-amber-900/20">
                <MessageSquare className="text-amber-600" size={24} aria-hidden="true" />
              </div>
              <h4 className="text-slate-900 dark:text-white font-bold mb-2">How can I help today?</h4>
              <p className="text-xs text-slate-600 dark:text-slate-300 dark:text-slate-400 mb-6 max-w-[240px] mx-auto">I can analyze pleadings, draft clauses, or check case law precedents.</p>
              ...
              <div className="space-y-2">
                {SUGGESTED_PROMPTS.map((prompt, idx) => (
                  ...
                  >
                    <prompt.icon size={14} className="text-amber-500" aria-hidden="true" />
                    {prompt.label}
                  </button>
                ))}
              </div>
          ...
          {chatHistory.map((msg) => (
            <div key={msg.id} className={`flex gap-3 ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
              <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold shadow-sm ${
                ...
                {msg.sender === 'user' ? 'YOU' : <Bot size={14} aria-hidden="true" />}
              </div>
              ...
              <div className={`max-w-[80%] space-y-1 ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  {msg.isRich ? (
                    <div className="whitespace-pre-line">
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-700/50 flex gap-2">
                        <button 
                          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 bg-slate-50 dark:bg-slate-900/50 text-[10px] font-bold text-slate-600 dark:text-slate-300 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 transition-all border border-slate-100 dark:border-slate-800"
                          aria-label="Copy AI response"
                        >
                            <FileText size={12} aria-hidden="true" /> Copy
                        </button>
                        <button 
                          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 bg-slate-50 dark:bg-slate-900/50 text-[10px] font-bold text-slate-600 dark:text-slate-300 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 transition-all border border-slate-100 dark:border-slate-800"
                          aria-label="Refine AI response"
                        >
                            <Sparkles size={12} aria-hidden="true" /> Refine
                        </button>
                      </div>
                    </div>
                  ) : (
                ...
                <span className="text-[9px] font-bold text-slate-500 dark:text-slate-300 dark:text-slate-600 uppercase px-1">
          ...
          {isChatLoading && (
            <div className="flex gap-3 animate-pulse">
              ...
              <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl rounded-tl-none px-4 py-3 shadow-sm flex items-center gap-3">
                <Loader2 size={14} className="animate-spin text-amber-600" aria-hidden="true" />
                <span className="text-xs font-bold text-slate-600 dark:text-slate-300 dark:text-slate-400 tracking-tight uppercase">Thinking...</span>
              </div>
            </div>
          )}

          {error && (
            <div className="mx-auto max-w-[90%] p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/50 rounded-xl text-red-600 dark:text-red-400 text-[11px] font-bold flex items-center gap-2" role="alert">
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-[0_-4px_12px_rgba(0,0,0,0.03)]">
          <div className="flex items-center gap-2 mb-3 px-1">
            <div className="flex items-center gap-1.5 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full border dark:border-slate-700">
                <Shield size={10} className="text-emerald-500" aria-hidden="true" />
                <span className="text-[9px] font-bold text-slate-600 dark:text-slate-300 dark:text-slate-400 uppercase tracking-tighter">Secure RAG Mode</span>
            </div>
            <span className="text-[10px] text-slate-300 dark:text-slate-700" aria-hidden="true">|</span>
            <span className="text-[10px] text-slate-500 dark:text-slate-300 dark:text-slate-600 font-medium">Context: {currentView}</span>
          </div>
          ...
          <p className="mt-2 text-[9px] text-center text-slate-500 dark:text-slate-300 dark:text-slate-600 font-medium">
